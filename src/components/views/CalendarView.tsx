      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <AnimatePresence initial={false}>
          {isSidebarOpen && (
            ...
            >
              ...
              <div>
                <button className="flex items-center gap-4 px-6 py-4 theme-glass hover:bg-theme-hover transition-all group rounded-full shadow-xl border-theme-border/50 bg-white/10">
        ...
        <main className="flex-1 overflow-hidden relative">
          <AnimatePresence mode="wait">
            ...
            >
              {viewType === 'week' ? (
                <WeekGrid days={weekDays} events={activeEvents} calendarColor={calendars.find(c => c.id === activeCalendarId)?.backgroundColor} />
              ) : viewType === 'day' ? (
                <WeekGrid days={[currentDate]} events={activeEvents} calendarColor={calendars.find(c => c.id === activeCalendarId)?.backgroundColor} />
              ) : viewType === 'month' ? (
                <MonthGrid days={monthDays} events={activeEvents} calendarColor={calendars.find(c => c.id === activeCalendarId)?.backgroundColor} />
              ) : (
                <AgendaView events={activeEvents} calendarColor={calendars.find(c => c.id === activeCalendarId)?.backgroundColor} />
              )}
            </motion.div>
          </AnimatePresence>
        </main>
...
const WeekGrid = ({ days, events, calendarColor }: { days: Date[], events: any[], calendarColor?: string }) => {
...
const MonthGrid = ({ days, events, calendarColor }: { days: Date[], events: any[], calendarColor?: string }) => {
  return (
...
const EventChip = ({ event, hourHeight, color }: { event: any, hourHeight: number, color?: string }) => {
...
const AgendaView = ({ events, calendarColor }: { events: any[], calendarColor?: string }) => {
